...
{
	Entity::Entity() :
		spriteKey_(""), pos_(Util::Vector2(0, 0)), isActive_(true), isClickable_(false), side_(-1), currTile_(nullptr)
	{
	...
	void Entity::Set_Side(int playerIndex)
	{
		side_ = playerIndex;
	}
	...
	bool Entity::Is_Colliding(const Entity *other)
	{
		if (currTile_ == other->currTile_) {
			return true;
		}
		return false;
	}
